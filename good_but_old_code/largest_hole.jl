using Distributed
@everywhere using LinearAlgebra, IterTools, SharedArrays
...
function nearest_neighbor_list(box)
    rough_range = box.ρInv()^(1/box.dim())
    nCellsPerSide = Int(fld(box.l, rough_range))
    cl = cell_list(box, nCellsPerSide)
    return mapslices(x -> nearest_neighbor(x, box, cl), box.particles; dims = 1)
end
    ...
    while !found
        ...
        for i in product(ntuple(i -> -surrounding_ind:surrounding_ind, box.dim())...)
            ...
            if size(current_neighbors, 2) > 0
                ...
                dist_temp = minimum(dists[dists .> 0])
...
@everywhere function particle_energy(particle, box, cl, hardcore)
    if nearest_neighbor(particle, box, cl) < hardcore 
        return Inf
    else
        return 0
    end
...
@everywhere function biased_energy(total_potential_energy, weight_func, dist_vec) 
    if total_potential_energy == Inf
        return Inf
    else
        return weight_func(minimum(dist_vec))
    end
end
...
function weight1(x)
    if x < 0.5
        return -40.616*0.5 + 20.96
    elseif x < 1.5
        return -40.616*x + 20.96
    else
        return Inf
    end
end 
